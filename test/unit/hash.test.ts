...
const badCases: any[] = [0, 42, NaN, null, undefined, true, false, [], {}, function() {}];
...
describe('Hash', () => {
	...
	describe('ofString', () => {
		...
		describe('returns 0 for', () => {
			for (const i in badCases) {
				it(`${prettyprint(badCases[i])}`, () => {
					expect(HashCode.ofString(<string><unknown>badCases[i])).to.equal(0);
				});
			}
		});
	});
	...
	describe('ofFunction', () => {
		describe('function keyword', () => {
			...
			it('returns same hashes for anonymous functions having same body', () => {
				expect(HashCode.ofFunction(function(k: string) { k.toUpperCase() }))
					.to.equal(HashCode.ofFunction(function(k: string) { k.toUpperCase() }));
			});

			it('returns same hashes for anonymous functions having same body but different source', () => {
				expect(HashCode.ofFunction(function(k: string) { 
					k.toUpperCase() 
				})).to.equal(HashCode.ofFunction(function(k: string) { k.toUpperCase() }));
			});
			...
			it('returns different hashes for differently named functions having same body', () => {
				...
				expect(HashCode.ofFunction(func1)).to.not.equal(HashCode.ofFunction(func2));
			});
		});
		...
		describe('fat-arrow', () => {
			...
			it('returns same hashes for anonymous functions having same source', () => {
				expect(HashCode.ofFunction((k: string) => { k.toUpperCase() }))
					.to.equal(HashCode.ofFunction((k: string) => { k.toUpperCase() }));
			});

			it('returns same hashes for anonymous functions having same body but different source', () => {
				expect(HashCode.ofFunction((k: string) => { 
					k.toUpperCase() 
				})).to.equal(HashCode.ofFunction((k: string) => { k.toUpperCase() }));
			});
